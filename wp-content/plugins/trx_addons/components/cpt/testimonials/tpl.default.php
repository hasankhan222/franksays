...
if ($query->found_posts > 0) {
	...
	?><div class="sc_testimonials sc_testimonials_<?php
		...
		while ( $query->have_posts() ) { $query->the_post();
			...
			$titles = '<div class="slide_info">
					<h4 class="sc_testimonials_item_author_title">'. trim(get_the_title()). '</h4>
					<div class="sc_testimonials_item_author_subtitle">'. esc_html($meta['subtitle']). '</div>
				</div>';
		...
		if ($args['slider']) {
			...
			if (!empty($args['slider_pagination_thumbs']))
				$args['slider_pagination_buttons'] = $pagination_bullets.$output;
