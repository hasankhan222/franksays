	...
	if (!empty($address) || !empty($phone) || !empty($email)) {
		?><div class="contacts_info"><?php
			if (!empty($email)) {
				...
				?><span class="contacts_email"><span class="label"><?php esc_html_e('GENERAL','eject'); ?></span><a href="mailto:<?php echo antispambot($email); ?>"><?php echo antispambot($email); ?></a></span><?php
				if ($columns) {
					?></div><?php
				}
			}
			if (!empty($phone) || !empty($address)) {
				...
				if (!empty($address)) {
					?><span class="contacts_address"><span class="label"><?php esc_html_e('LOCATION','eject'); ?></span><?php echo str_replace('|', "<br>", $address); ?></span><?php
				}
				if (!empty($phone)) {
					?><span class="contacts_phone"><span class="label"><?php esc_html_e('Phone','eject'); ?></span><a href="<?php echo esc_attr(trx_addons_get_phone_link($phone)); ?>" class="phone"><?php echo wp_kses_data($phone); ?></a></span><?php
				}
				if ($columns) {
					?></div><?php
				}
