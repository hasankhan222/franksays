<div class="front_page_section front_page_section_blog<?php
			$eject_scheme = eject_get_theme_option('front_page_blog_scheme');
			if (!eject_is_inherit($eject_scheme)) echo ' scheme_'.esc_attr($eject_scheme);
			echo ' front_page_section_paddings_'.esc_attr(eject_get_theme_option('front_page_blog_paddings'));
		?>"<?php
		$eject_css = '';
		$eject_bg_image = eject_get_theme_option('front_page_blog_bg_image');
...
?>><?php
	// Add anchor
	$eject_anchor_icon = eject_get_theme_option('front_page_blog_anchor_icon');	
	$eject_anchor_text = eject_get_theme_option('front_page_blog_anchor_text');	
	...
	<div class="front_page_section_inner front_page_section_blog_inner<?php
			if (eject_get_theme_option('front_page_blog_fullheight'))
				echo ' eject-full-height sc_layouts_flex sc_layouts_columns_middle';
			?>"<?php
			$eject_css = '';
			$eject_bg_mask = eject_get_theme_option('front_page_blog_bg_mask');
			$eject_bg_color = eject_get_theme_option('front_page_blog_bg_color');
	...
	?>>
		<div class="front_page_section_content_wrap front_page_section_blog_content_wrap content_wrap">
			<?php
			// Caption
			$eject_caption = eject_get_theme_option('front_page_blog_caption');
			if (!empty($eject_caption) || (current_user_can('edit_theme_options') && is_customize_preview())) {
				?><h2 class="front_page_section_caption front_page_section_blog_caption front_page_block_<?php echo !empty($eject_caption) ? 'filled' : 'empty'; ?>"><?php echo wp_kses_post($eject_caption); ?></h2><?php
			}
		
			// Description (text)
			$eject_description = eject_get_theme_option('front_page_blog_description');
			if (!empty($eject_description) || (current_user_can('edit_theme_options') && is_customize_preview())) {
				?><div class="front_page_section_description front_page_section_blog_description front_page_block_<?php echo !empty($eject_description) ? 'filled' : 'empty'; ?>"><?php echo wpautop(wp_kses_post($eject_description)); ?></div><?php
			}
			...
			?><div class="front_page_section_output front_page_section_blog_output"><?php 
				if (is_active_sidebar('front_page_blog_widgets')) {
					...
					if (!eject_exists_trx_addons())
						...
						eject_customizer_need_widgets_message('front_page_blog_caption', 'ThemeREX Addons - Blogger');
				}
			?></div>
		</div>
	</div>
