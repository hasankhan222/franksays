...
if (!function_exists('eject_hovers_theme_setup3')) {
	add_action( 'after_setup_theme', 'eject_hovers_theme_setup3', 3 );
	function eject_hovers_theme_setup3() {

		// Add 'Buttons hover' option
		eject_storage_set_array_after('options', 'border_radius', array(
			'button_hover' => array(
				"title" => esc_html__("Button's hover", 'eject'),
				"desc" => wp_kses_data( __('Select hover effect to decorate all theme buttons', 'eject') ),
				"std" => 'default',
				"options" => array(
					'default'		=> esc_html__('Fade',				'eject'),
				),
				"type" => "select"
			),
			'image_hover' => array(
				"title" => esc_html__("Image's hover", 'eject'),
				"desc" => wp_kses_data( __('Select hover effect to decorate all theme images', 'eject') ),
				"std" => 'icon',
				"override" => array(
					'mode' => 'page',
				...
				"options" => array(
					'dots'	=> esc_html__('Dots',	'eject'),
					'icon'	=> esc_html__('Icon',	'eject'),
					'icons'	=> esc_html__('Icons',	'eject'),
					'zoom'	=> esc_html__('Zoom',	'eject'),
					'fade'	=> esc_html__('Fade',	'eject'),
					'slide'	=> esc_html__('Slide',	'eject'),
					'pull'	=> esc_html__('Pull',	'eject'),
					'border'=> esc_html__('Border',	'eject')
				),
				"type" => "select"
			) )
		);
...
if (!function_exists('eject_hovers_theme_setup9')) {
	add_action( 'after_setup_theme', 'eject_hovers_theme_setup9', 9 );
	function eject_hovers_theme_setup9() {
		add_action( 'wp_enqueue_scripts',		'eject_hovers_frontend_scripts', 1010 );
		add_filter( 'eject_filter_localize_script','eject_hovers_localize_script' );
		add_filter( 'eject_filter_merge_scripts',	'eject_hovers_merge_scripts' );
		add_filter( 'eject_filter_merge_styles',	'eject_hovers_merge_styles' );
		add_filter( 'eject_filter_get_css', 		'eject_hovers_get_css', 10, 4 );
	}
}
	
// Enqueue hover styles and scripts
if ( !function_exists( 'eject_hovers_frontend_scripts' ) ) {
	//Handler of the add_action( 'wp_enqueue_scripts', 'eject_hovers_frontend_scripts', 1010 );
	function eject_hovers_frontend_scripts() {
		if ( eject_is_on(eject_get_theme_option('debug_mode')) && eject_get_file_dir('theme-specific/theme.hovers/theme.hovers.js')!='' )
			wp_enqueue_script( 'eject-hovers', eject_get_file_url('theme-specific/theme.hovers/theme.hovers.js'), array('jquery'), null, true );
		if ( eject_is_on(eject_get_theme_option('debug_mode')) && eject_get_file_dir('theme-specific/theme.hovers/theme.hovers.css')!='' )
			wp_enqueue_style( 'eject-hovers',  eject_get_file_url('theme-specific/theme.hovers/theme.hovers.css'), array(), null );
	}
}

// Merge hover effects into single js
if (!function_exists('eject_hovers_merge_scripts')) {
	//Handler of the add_filter( 'eject_filter_merge_scripts', 'eject_hovers_merge_scripts' );
	function eject_hovers_merge_scripts($list) {
...
if (!function_exists('eject_hovers_merge_styles')) {
	//Handler of the add_filter( 'eject_filter_merge_styles', 'eject_hovers_merge_styles' );
	function eject_hovers_merge_styles($list) {
...
if (!function_exists('eject_hovers_localize_script')) {
	//Handler of the add_filter( 'eject_filter_localize_script','eject_hovers_localize_script' );
	function eject_hovers_localize_script($arr) {
		$arr['button_hover'] = eject_get_theme_option('button_hover');
		return $arr;
	}
}

// Add hover icons on the featured image
if ( !function_exists('eject_hovers_add_icons') ) {
	function eject_hovers_add_icons($hover, $args=array()) {
					...
					echo apply_filters( 'woocommerce_loop_add_to_cart_link',
										'<a rel="nofollow" href="' . esc_url($product->add_to_cart_url()) . '" 
														...
														class="shop_cart icon-cart-2 button add_to_cart_button'
																...
																. '">'
											. ($hover == 'shop_buttons' ? esc_html__('Buy now', 'eject') : '')
										. '</a>',
										$product );
				}
				?><a href="<?php echo esc_url(is_object($args['cat']) ? get_term_link($args['cat']->slug, 'product_cat') : get_permalink()); ?>" aria-hidden="true" class="shop_link button icon-link"><?php
					if ($hover == 'shop_buttons') {
						if (is_object($args['cat']))
							esc_html_e('View products', 'eject');
						else
							esc_html_e('Details', 'eject');
					}
				?></a>
			</div>
			<?php
		...
		// Hover style 'Icon'
			...
			?><div class="icons"><a href="<?php the_permalink(); ?>" aria-hidden="true" class="icon-unlink"></a></div><?php
...
// Add styles into CSS
if ( !function_exists( 'eject_hovers_get_css' ) ) {
	//Handler of the add_filter( 'eject_filter_get_css', 'eject_hovers_get_css', 10, 4 );
	function eject_hovers_get_css($css, $colors, $fonts, $scheme='') {
